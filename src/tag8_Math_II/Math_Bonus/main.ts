...
// Функция для нахождения рекордных температур
function getRecordTemperatures(weather: Weather): { minTemp: number; maxTemp: number } {
    const minTemperatures = weather.data.map(entry => entry.temperature_min);
    const maxTemperatures = weather.data.map(entry => entry.temperature_max);
...
const { minTemp, maxTemp } = getRecordTemperatures(weather);
console.log(`Record low temperature of the year: ${minTemp}°C`);
console.log(`Record high temperature of the year: ${maxTemp}°C`);